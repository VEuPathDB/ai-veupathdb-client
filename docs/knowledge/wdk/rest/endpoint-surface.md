...
| POST | `/users/{userId}/steps/{stepId}/reports/standard` | Run the step and page its records | `{reportConfig, viewFilters}` | `Answer` with `records` and `meta` | `strategy_api/base.py:_standard_report`, used by `reports.py:get_step_records` and `get_step_count`; `viewFilters` is sent only when `get_step_records` or `get_step_answer` is given `view_filters` |
